...
use crate::{
    ...
    END_AT, TARGET_EPOCH_REWARDS, TREASURY,
};

/// Reset sets up the Mars program for the next epoch. Its responsibilities include:
...
/// - Can only succeed if END_AT has not reached.
...
///   stays within the guaranteed bounds of 0 ≤ R ≤ 20 MARS/min.
/// - The reward rate is dynamically adjusted based on last epoch's actual reward rate (proxy for hashpower) to
///   target an average supply growth rate of 10 MARS/min.
pub fn process_reset<'a, 'info>(
    ...
    if clock.unix_timestamp.lt(&START_AT) {
        return Err(MarsError::NotStarted.into());
    }
    
    if clock.unix_timestamp.ge(&END_AT) {
        return Err(MarsError::HasEnded.into());
    }
    ...
    if clock.unix_timestamp.lt(&threshold) {
        return Err(MarsError::ResetTooEarly.into());
    }
...
///
/// The new rate is then smoothed by a constant factor to avoid large fluctuations. In Mars's case,
...
pub(crate) fn calculate_new_reward_rate(current_rate: u64, epoch_rewards: u64) -> u64 {
    ...
    // Prevent reward rate from dropping below 10 or exceeding BUS_EPOCH_REWARDS and return.
    // new_rate_smoothed.max(1).min(BUS_EPOCH_REWARDS)
    new_rate_smoothed.max(10).min(BUS_EPOCH_REWARDS)
}
...
mod tests {
    ...
    fn test_calculate_new_reward_rate_lower_fuzz() {
        ...
        for _ in 0..FUZZ_SIZE {
            let current_rate: u64 = rng.sample(Uniform::new(10, BUS_EPOCH_REWARDS));
    ...
    fn test_calculate_new_reward_rate_higher_fuzz() {
        ...
        for _ in 0..FUZZ_SIZE {
            let current_rate: u64 = rng.sample(Uniform::new(10, BUS_EPOCH_REWARDS));
            let actual_rewards: u64 = rng.sample(Uniform::new(10, TARGET_EPOCH_REWARDS));
            let new_rate = calculate_new_reward_rate(current_rate, actual_rewards);
            assert!(new_rate.gt(&current_rate));
        }
    }
    ...
    fn test_calculate_new_reward_rate_max_smooth() {
        let current_rate = 10 * 1000;
        let new_rate = calculate_new_reward_rate(current_rate, 10);
    ...
    fn test_calculate_new_reward_rate_min_inputs() {
        let new_rate = calculate_new_reward_rate(10, 10);
        assert!(new_rate.eq(&10u64.saturating_mul(SMOOTHING_FACTOR)));
    }
}
