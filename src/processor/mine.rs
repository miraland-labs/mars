...
use crate::{
    ...
    EPOCH_DURATION, START_AT, END_AT,
};

/// Mine is the primary workhorse instruction of the Mars program. Its responsibilities include:
...
pub fn process_mine<'a, 'info>(
    ...
    if clock.unix_timestamp.ge(&threshold) {
        return Err(MarsError::NeedsReset.into());
    }
    ...
    bus.rewards = bus
        ...
        .ok_or(MarsError::BusRewardsInsufficient)?;
...
pub(crate) fn validate_hash(
    ...
    if sol_memcmp(hash.as_ref(), hash_.as_ref(), HASH_BYTES) != 0 {
        return Err(MarsError::HashInvalid.into());
    }

    // Validate hash difficulty
    if hash.gt(&difficulty) {
        return Err(MarsError::DifficultyNotSatisfied.into());
    }
