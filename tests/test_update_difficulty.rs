...
async fn test_update_difficulty() {
    ...
    let ix = mars::instruction::update_difficulty(payer.pubkey(), new_difficulty.into());
    let tx = Transaction::new_signed_with_payer(&[ix], Some(&payer.pubkey()), &[&payer], blockhash);
    let res = banks.process_transaction(tx).await;
    assert!(res.is_ok());
...
async fn test_update_difficulty_bad_signer() {
    ...
    // Submit update difficulty ix
    let new_difficulty = KeccakHash::new_unique();
    let ix = mars::instruction::update_difficulty(alt_payer.pubkey(), new_difficulty.into());
...
async fn test_update_difficulty_not_enough_accounts() {
    ...
    let new_difficulty = KeccakHash::new_unique();
    let mut ix = mars::instruction::update_difficulty(payer.pubkey(), new_difficulty.into());
